...
class BotDB:
    ...
    def get_resume(self, user_name):
        ...
        result = self.cursor.execute(
            "SELECT firs_name, city, spesiolization, salary, experience, employment, schedule FROM `resume` WHERE `firs_name` = ?",
            (user_name,))
    ...
    def set_spesiolization(self, user_name, spesiolization):
        ...
        result = self.cursor.execute("UPDATE resume SET spesiolization = ? WHERE firs_name = ?",
                                     (spesiolization, user_name,))
        return self.conn.commit()

    def set_salary(self, user_name, salary):
        """Set salary in my search qwery"""
        self.cursor.execute("UPDATE resume SET salary = ? WHERE firs_name = ?", (salary, user_name,))
        return self.conn.commit()

    def set_experience(self, user_name, experience):
        """Set experience in my search qwery"""
        self.cursor.execute("UPDATE resume SET experience = ? WHERE firs_name = ?", (experience, user_name,))
        return self.conn.commit()

    def set_employment(self, user_name, employment):
        """Set employment in my search qwery"""
        self.cursor.execute("UPDATE resume SET employment = ? WHERE firs_name = ?", (employment, user_name,))
        return self.conn.commit()

    def set_schedule(self, user_name, schedule):
        """Set schedule in my search qwery"""
        self.cursor.execute("UPDATE resume SET schedule = ? WHERE firs_name = ?", (schedule, user_name,))
        return self.conn.commit()
